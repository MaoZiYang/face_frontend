...
angular.module('admin')
    .factory('path', function (commonUtil) {
        return {
            ...
            login: "/face-admin-ajax/a/login",
            //退出logout      请求方式#地址：post # /a/logout
            logout: "/face-admin-ajax/a/logout",
            ...
            manager: function (mid) {
                if (mid == undefined || mid == "") {
                    ...
                    return "/face-admin-ajax/a/u/manager";
                } else {
                    // 根据ID查找用户:Get  /a/u/manager/{mid} // 修改用户：Put  /a/u/manager/{mid} // 删除用户: Delete   /a/u/manager/{mid}
                    return "/face-admin-ajax/a/u/manager/" + mid;
                }

            },
            // 增加用户：Post  /a/u/manager
            manager_list: "/face-admin-ajax/a/u/manager/",
            // 批量获取用户详细信息
            manager_multi_detail: function (params) {
                return "/face-admin-ajax/a/u/multi/manager" + commonUtil.concactArrayParam("ids", params);

            },
            // 修改密码
            changePwd: "/face-admin-ajax/a/u/pwd",
            /**
             * 角色管理
             * @param rid
             * @returns {*}
            ...
            role: function (rid) {
                ...
                if (rid == undefined || rid == "") {
                    //增加角色
                    return "/face-admin-ajax/a/u/role";
                } else {
                    //修改角色 //删除角色
                    return "/face-admin-ajax/a/u/role/" + rid;
                }

            },
            //查找角色列表
            role_list: "/face-admin-ajax/a/u/role/",
            role_multi_detail: function (params) {
                //批量获取角色详细信息
                return "/face-admin-ajax/a/u/multi/role" + commonUtil.concactArrayParam("ids", params);

            },
            role_module: function (rid) {
                // 返回单个角色及权限
                return "/face-admin-ajax/a/u/role/" + rid + "/module"
            },
            role_manager: function (rid) {
                return "/face-admin-ajax/a/u/role/" + rid + "/manager"
            },
            /**
             * 模块管理
             * @param mid
            ...
            module: function (mid) {
                if (mid == undefined || mid == "") {
                    //增加模块
                    return "/face-admin-ajax/a/u/module";
                } else {
                    //根据ID查找模块 //修改模块 //删除模块
                    return "/face-admin-ajax/a/u/module/" + mid
                }
            },
            //查询模块列表
            module_list: "/face-admin-ajax/a/u/module/",
            //批量获取模块详细信息
            module_multi_detail: function (params) {
                return "/face-admin-ajax/a/u/multi/module" + commonUtil.concactArrayParam("ids", params);
            ...
            article: function (aid) {
                if (aid == undefined || aid == "") {
                    return "/face-admin-ajax/a/u/article";
                } else {
                    return "/face-admin-ajax/a/u/article/" + aid;
                }
            }
        }
    })
